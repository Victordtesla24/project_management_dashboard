    ...
    def doctestify(self, input):
        ...
        for line in textwrap.dedent(input).splitlines():
            if line.strip() == "":
                ...
                line.startswith((" ", "except:", "except ", "finally:", "else:", "elif "))
                or lines
...
class Test(TestCase):
    ...
    def test_scope_class(self):
        ...
        assert len(doctest_scopes) == 1

        module_scope = module_scopes[0]
        doctest_scope = doctest_scopes[0]

        assert isinstance(doctest_scope, DoctestScope)
        assert isinstance(doctest_scope, ModuleScope)
        assert not isinstance(doctest_scope, FunctionScope)
        assert not isinstance(module_scope, DoctestScope)

        assert "m" in module_scope
        assert "doctest_stuff" in module_scope

        assert "d" in doctest_scope

        assert len(function_scopes) == 1
        assert "f" in function_scopes[0]
    ...
    def test_nested_doctest_ignored(self):
        ...
        assert "m" in module_scope
        assert "doctest_stuff" in module_scope
        assert "function_in_doctest" in doctest_scope

        assert len(function_scopes) == 2

        assert "f" in function_scopes[0]
        assert "df" in function_scopes[1]
    ...
    def test_global_module_scope_pollution(self):
        ...
        assert "f" in function_scopes[0]
        assert "df" in function_scopes[1]
        assert "m" in function_scopes[1]

        assert "m" not in module_scope
