...
class ConfigManager:
    ...
    REQUIRED_SECTIONS = {
        ...
        "websocket": {"host": str, "port": int, "ssl": bool},
        "database": {"host": str, "port": int, "name": str, "user": str, "password": str},
        "logging": {"level": str, "file": str, "format": str},
        "ui": {"theme": str, "refresh_interval": int, "max_datapoints": int, "layout": dict},
    }
        ...
        for section, schema in self.REQUIRED_SECTIONS.items():
            ...
            if section == "metrics":
                ...
                if "max_datapoints" not in retention:
                    raise SchemaValidationError(
                        "Missing required field in metrics.retention: max_datapoints"
                    )
                if not isinstance(retention["max_datapoints"], int):
                    raise SchemaValidationError(
                        "metrics.retention.max_datapoints must be an integer"
                    )
                ...
                for rule in alert_rules:
                    ...
                    for field in required_fields:
                        if field not in rule:
                            raise SchemaValidationError(
                                f"Missing required field in alert rule: {field}"
                            )
                ...
                if "interval" not in aggregation:
                    raise SchemaValidationError(
                        "Missing required field in metrics.aggregation: interval"
                    )
                if "functions" not in aggregation:
                    raise SchemaValidationError(
                        "Missing required field in metrics.aggregation: functions"
                    )
        ...
        for rule in rules:
            if not all(k in rule for k in ["metric", "threshold", "duration", "severity"]):
                return ValidationResult(
                    False, ["Missing required fields in alert rule: severity, threshold, duration"]
                )
